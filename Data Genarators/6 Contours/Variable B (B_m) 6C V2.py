...
DX1, DY1 = make_gaussian_quantiles(n_samples=40000,cov=5, n_features=2, n_classes=7)
...
CX2[:,1] = np.interp(CX1[:,1], (np.min(CX1[:,1]), np.max(CX1[:,1])), (-20, 109))

DX2[:,0] = np.interp(DX1[:,0], (np.min(DX1[:,0]), np.max(DX1[:,0])), (79, 189))
DX2[:,1] = np.interp(DX1[:,1], (np.min(DX1[:,1]), np.max(DX1[:,1])), (80, 219))
...
dfB.to_csv(r'E:\Research\CHI Project\Controlled Experiment\4 variables\Data\Bm25.csv')
